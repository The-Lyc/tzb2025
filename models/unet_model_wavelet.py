...
class UNet_F_wave(nn.Module):
    def __init__(self, cfg: DictConfig):
       ...
       # self.saa = SAA(cfg)
        ...
        if self.mode == 1:
            if self.layer == 4:
                self.resize_conv = nn.Conv2d(in_channels=1, out_channels=1, kernel_size=4, stride=4, padding=0)
    ...
    def forward(self, x):
       ...
       # x_interp = self.interpolate(x)
        ...
        # x = self.saa(x)
        x_resized = self.resize_conv(x_resized)
